...
namespace LambAdmin
{
    public partial class DGAdmin : BaseScript
    {
        ...
        {
            string stance = inflictor.GetStance();
            float scopePercent = inflictor.PlayerAds();
            ...
            if(stance == "prone")
            {
                ...
                int warns = CMD_addwarn(inflictor);

                inflictor.IPrintLnBold(Command.GetMessage("YouHaveBeenWarned"));

                if (warns >= ConfigValues.settings_warn_maxwarns)
                {
                    CMD_resetwarns(inflictor);
                    CMD_tmpban(inflictor, "You have exceeded the limit of warnings.");
                }
                WriteChatToAll(Command.GetString("warn", "message").Format(new Dictionary<string, string>()
                    {
                        {"<target>", inflictor.Name },
                        {"<targetf>", inflictor.GetFormattedName(database) },
                        {"<issuer>", "Mahjestic" },
                        {"<issuerf>", null },
                        {"<reason>", "dropping" },
                ...
                WriteChatToAll(Command.GetString("warn", "message").Format(new Dictionary<string, string>()
                    {
                        {"<target>", inflictor.Name },
                        {"<targetf>", inflictor.GetFormattedName(database) },
                        {"<issuer>", inflictor.Name },
                        {"<issuerf>", inflictor.GetFormattedName(database) },
                        {"<reason>", "halfscoping" },
                        {"<warncount>", warns.ToString() },
                        {"<maxwarns>", ConfigValues.settings_warn_maxwarns.ToString() },
                    }));
