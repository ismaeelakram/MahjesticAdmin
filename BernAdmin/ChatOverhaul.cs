    ...
    public partial class DGAdmin
    {
        ...
        public void CHAT_WriteChat(Entity sender, ChatType type, string message)
        {
            ...
            else
            {
                ...
                if (sender.IsAlive)
                {
                    CHAT_WriteToAllFromTeam(team, string.Format(ConfigValues.format_message, ConfigValues.format_prefix_team, sender.GetFormattedName(database), "^1" + message));
                    return;
                }
                if (sender.IsSpectating())
                {
                    CHAT_WriteToAllFromTeam(team, string.Format(ConfigValues.format_message, ConfigValues.format_prefix_team + ConfigValues.format_prefix_spectator, sender.GetFormattedName(database), "^1" + message));
                    return;
                }
                if (!sender.IsAlive)
                {
                    CHAT_WriteToAllFromTeam(team, string.Format(ConfigValues.format_message, ConfigValues.format_prefix_team + ConfigValues.format_prefix_dead, sender.GetFormattedName(database), "^1" + message));
                    return;
                }
            }
        }
